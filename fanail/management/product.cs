...
namespace fanail.management
{
	public partial class product : Form
	{
		...
		{
			model.SqlExecute("update tb_product set pro_status=0 where pro_id='" + id + "'");
			loadData();
			cbb_producttype();
			cbb_unit();
		...
		private bool checkIsnullText()
		{
			...
			if (guna2DataGridView1.Rows.Count == 0)
			{
				...
				else
				{
					...
					{
						model.Get_ID("SELECT * FROM tb_product WHERE pro_id='" + guna2TextBox2.Text + "'");
						if (model.b == false)
						{

							model.SqlExecute(@"Update tb_product set pro_name=N'" + guna2TextBox4.Text + "',type_id=N'" + cbbname.SelectedValue + "',unit_id=N'" + guna2ComboBox1.SelectedValue + "',pro_orderPrice=N'" + guna2TextBox3.Text + "'," +
						 "pro_sellPrice=N'" + guna2TextBox1.Text + "',pro_status=1,pro_message=N'" + guna2TextBox5.Text + "'" +
						  " where pro_id='" + guna2TextBox2.Text + "'");
							AlertMessage alert = new AlertMessage();
							alert.TopMost = true;
